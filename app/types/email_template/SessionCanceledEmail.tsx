...
export interface SessionCanceledProps {
    recipientName: string;   // mentee 的名字
...
const SessionCanceledEmail: React.FC<SessionCanceledProps> = ({
    ...
    <Html>
        <Head />
        <Preview>Your session with {mentorName} was canceled</Preview>
        <Body style={main}>
            <Container style={container}>
                <Text style={h1}>Session Canceled</Text>
                ...
                <Text style={text}>
                    We’re sorry to let you know that your session with <strong>{mentorName}</strong> (ID: <em>{appointmentId}</em>) scheduled on{' '}
                    <strong>{sessionDate} at {sessionTime}</strong> has been <strong>canceled</strong>.
                </Text>

                {cancelReason && (
                    <Section style={boxContainer}>
                        <Text style={h2}>Reason Given:</Text>
                        <Text style={text}>{cancelReason}</Text>
                    </Section>
                )}

                <Text style={text}>
                    We will process your refund within 3–5 business days. If you have any questions, please reply to this email or contact{' '}
                    <Link
